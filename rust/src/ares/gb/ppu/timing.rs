...
impl PPU {
    pub fn can_access_vram(&self) -> bool {
        !self.status.display_enable
            || self.history.mode.get_bits(4, 6).x() != 3
                && (self.history.mode.get_bits(4, 6).x() != 2 || self.status.lx.x() >> 2 != 20)
    }

    pub fn can_access_oam(&self) -> bool {
        !self.status.display_enable
            || (self.status.dma_active
                && self.status.dma_clock.x() < 8
                && self.history.mode.get_bits(4, 6).x() != 2
                && self.history.mode.get_bits(4, 6).x() != 3
                && (self.status.ly == 0 || self.status.lx.x() >> 2 != 0))
    }
}
