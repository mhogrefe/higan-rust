...
auto APU::main() -> void {
  ...
  u32 triangleOutput = 0; triangle.clock();
  u32 noiseOutput = 0; noise.clock();
  u32 dmcOutput = 0; dmc.clock();
...
auto APU::writeIO(n16 address, n8 data) -> void {
  ...
  auto d = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
  std::cout << d << std::hex << " " << address << " " << (n32) data << " " << (n32) enabledChannels << std::endl;
